...
class TestAmpliconPrepRoundTrip(unittest.TestCase):
    ...
    def test_amplicon_515f806r_celeste(self):
        self._assert_roundtrips("good_amplicon_515f806r_celeste.tsv")

    def test_amplicon_515f806r_prep(self):
        self._assert_roundtrips("good_amplicon_515f806r_prep.tsv")

    def test_amplicon_celeste_16s(self):
        self._assert_roundtrips("good_amplicon_celeste_16s.txt")

    def test_amplicon_katharoseq_16s(self):
        self._assert_roundtrips("good_amplicon_katharoseq_16s.txt")
    ...
    def test_open_file_accepts_a_prep_template(self):
        ...
        sheet = DATA_DIR / "good_amplicon_celeste_16s.txt"
...
class TestAmpliconPrepProjection(unittest.TestCase):
    ...
    def test_katharoseq_controls_are_typed_and_measured(self):
        # The sheet names its controls "katharo.*" in lower case and records a
        # rack and a cell count for each.
        conn = self._load("good_amplicon_katharoseq_16s.txt")
        try:
            counts = dict(
                conn.execute(
        ...
        self.assertEqual(
            counts,
            {
                "standard": 357,
                "extraction_blank": 15,
                "katharoseq_cells_positive_control": 12,
            },
        )
        self.assertEqual(katharoseq_rows, 12)

    def test_numbered_blank_prefix_is_not_assumed_to_be_a_control(self):
        # This layout writes "BLANK2.2A" rather than "BLANK.<something>".
        # Whether a numbered prefix marks a blank is a question about the
        # sheet's convention; typing reads the name literally rather than
        # guessing, so these load as ordinary samples until that is settled.
        conn = self._load("good_amplicon_515f806r_celeste.tsv")
        try:
            counts = dict(
                conn.execute(
                    "SELECT st.name, COUNT(*) FROM input_sample i "
            ...
            conn.close()
        self.assertEqual(counts, {"standard": 384})

    def test_run_and_plate_facts_are_typed_once(self):
        # Facts constant across a run or a plate are stored once at that grain
        # rather than repeated on all 384 rows.
        conn = self._load("good_amplicon_celeste_16s.txt")
        try:
            run = conn.execute(
    ...
    def test_barcode_roster_reads_without_accessions(self):
        ...
        try:
            ...
            conn.close()
        self.assertEqual(len(roster), 384)
